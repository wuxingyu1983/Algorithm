// https://onlinejudge.org/index.php?option=onlinejudge&Itemid=8&page=show_problem&problem=3637

#include <cmath>
#include <cstdio>
#include <vector>
#include <iostream>
#include <algorithm>
#include <stdio.h>
#include <string.h>
#include <iostream>
#include <fstream>
#include <set>
#include <queue>
#include <stack>
#include <unordered_map>
#include <utility>

using namespace std;

class Block
{
public:
    int l, m;

    Block() : l(0), m(0) {}
};

bool cmp(Block &a, Block &b)
{
    bool bRet = false;
 
    if (a.l < b.l)
    {
        bRet = true;
    }
    else if (a.l == b.l)
    {
        if (a.m < b.m)
        {
            bRet = true;
        }
    }

    return bRet;
}

// 合并左右子树的最大值
void push_up(vector<int> &tree, int p)
{
    tree[p] = max(tree[p << 1], tree[p << 1 | 1]);
}

void updateMax(vector<int> &tree, int idx, int c, int l, int r, int p)
{
    if (idx == l && idx == r)
    {
        tree[p] = c;
        return;
    }

    int mid = (l + r) >> 1;
    if (idx <= mid)
        updateMax(tree, idx, c, l, mid, p << 1);
    else
        updateMax(tree, idx, c, mid + 1, r, p << 1 | 1);
    push_up(tree, p);
}

// 区间查询：查询[L, R]内的最大值
int getMax(vector<int> &tree, int L, int R, int l, int r, int p)
{
    if (L > R)
        return 0;
    if (L <= l && r <= R)
        return tree[p]; // 完全覆盖
    int mid = (l + r) >> 1;
    int res = 0;
    if (L <= mid)
        res = max(res, getMax(tree, L, R, l, mid, p << 1));
    if (R > mid)
        res = max(res, getMax(tree, L, R, mid + 1, r, p << 1 | 1));
    return res;
}

int main()
{
    ios_base::sync_with_stdio(false);
    cin.tie(NULL);

    vector<int> tree(500);
    vector<Block> blocks;

    while (true)
    {
        int n;
        cin >> n;

        if (0 == n)
        {
            cout << "*\n";
            break;
        }

        tree.assign(500, 0);
        blocks.resize(n);

        for (size_t i = 0; i < n; i++)
        {
            int l, m;
            cin >> l >> m;

            blocks[i].l = l;
            blocks[i].m = m;
        }
        
        sort(blocks.begin(), blocks.end(), cmp);

        int ans = 0;

        for (size_t i = 0; i < n; i++)
        {
            int len = getMax(tree, 1, blocks[i].m, 1, 100, 1);
            updateMax(tree, blocks[i].m, len + 1, 1, 100, 1);

            ans = max(ans, len + 1);
        }

        cout << ans << "\n";
    }
    
    return 0;
}
